...
def generateDataMapAirSim(folder):
    ...
    current_df = pd.read_csv(os.path.join(folder, 'data.csv'), sep=',')
    current_df = current_df.rename(columns=({'Unnamed: 0':'image_path','dist_0':'distance','ABS_RX':'steering_angle','ABS_Y':'speed'}))


    for i in range(1, current_df.shape[0] - 1, 1):
        previous_state = list(current_df.iloc[i-1][['distance', 'steering_angle', 'speed']])
        current_label = list((current_df.iloc[i][['steering_angle']] + current_df.iloc[i-1][['steering_angle']] + current_df.iloc[i+1][['steering_angle']]) / 3.0)
        
        image_filepath = os.path.join(os.path.join(folder, current_df.iloc[i]['image_path']),'.jpg').replace('/.', '.')
...
def generatorForH5py(data_mappings, chunk_size=20):
    """
...
def preprocess(folders, output_directory, train_eval_test_split):
